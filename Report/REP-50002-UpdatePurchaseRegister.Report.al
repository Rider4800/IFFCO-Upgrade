report 50002 "Update Purchase Register"
{
    ...
    begin
        ...
        //16767 WITH lPurchInvLine DO BEGIN
        lPurchInvHeader.GET(lPurchInvLine."Document No.");
        //IF GenJnlTemplate.GET(lPurchInvHeader."Gen. Journal Template Code") THEN;

        IF Loc.GET(lPurchInvLine."Location Code") THEN;
        ...
        PurchaseRegister."Source Document No." := lPurchInvLine."Document No.";
        IF (lPurchInvLine.Type = Type::"G/L Account") AND (lPurchInvLine."No." = VPG."Invoice Rounding Account") THEN BEGIN
            ...
            PurchaseRegister1.SETRANGE("Source Document No.", lPurchInvLine."Document No.");
            ...
            IF lPurchInvHeader."Currency Code" <> '' THEN BEGIN
                ...
                PurchaseRegister1."Round Off Amount" := AmtToVendor.GetAmttoVendorPostedDoc(lPurchInvLine."Document No.") / lPurchInvHeader."Currency Factor";
                PurchaseRegister1."Net Amount" += AmtToVendor.GetAmttoVendorPostedDoc(lPurchInvLine."Document No.") / lPurchInvHeader."Currency Factor";
            END ELSE BEGIN
                //16767 PurchaseRegister1."Round Off Amount" := "Amount To Vendor";
                //16767 PurchaseRegister1."Net Amount" += "Amount To Vendor";
                PurchaseRegister1."Round Off Amount" := AmtToVendor.GetAmttoVendorPostedDoc(lPurchInvLine."Document No.");
                PurchaseRegister1."Net Amount" += AmtToVendor.GetAmttoVendorPostedDoc(lPurchInvLine."Document No.");
            END;
        ...
        END ELSE BEGIN
            ...
            PurchaseRegister."Source Line No." := lPurchInvLine."Line No.";
            ...
            PurchaseRegister."Source Line Description" := lPurchInvLine.Description;
            ...
            PurchaseRegister."Source No." := lPurchInvLine."Buy-from Vendor No.";
            ...
            PurchaseRegister."Gen. Bus. Posting Group" := lPurchInvLine."Gen. Bus. Posting Group";
            IF GBPG.GET(lPurchInvLine."Gen. Bus. Posting Group") THEN;
            PurchaseRegister."GBPG Description" := GBPG.Description;
            IF GPPG.GET(lPurchInvLine."Gen. Prod. Posting Group") THEN;
            ...
            END ;
            */
            PurchaseRegister."Outward Location Code" := lPurchInvLine."Location Code";
            ...
            IF (lPurchInvLine.Type = Type::Item) AND (lPurchInvLine."No." <> '') THEN BEGIN
                Item.GET(lPurchInvLine."No.");
            ...
            //16767 New Code add
            IF lPurchInvLine.Type = lPurchInvLine.Type::" " then
                PurchaseRegister.Type := PurchaseRegister.Type::" "
            else
                IF lPurchInvLine.Type = lPurchInvLine.Type::"Charge (Item)" then
                    PurchaseRegister.Type := PurchaseRegister.Type::"Charge (Item)"
                else
                    IF lPurchInvLine.Type = lPurchInvLine.Type::"Fixed Asset" then
                        PurchaseRegister.Type := PurchaseRegister.Type::"Fixed Asset"
                    else
                        IF lPurchInvLine.Type = lPurchInvLine.Type::"G/L Account" then
                            PurchaseRegister.Type := PurchaseRegister.Type::"G/L Account"
                        else
                            IF lPurchInvLine.Type = lPurchInvLine.Type::Item then
                                PurchaseRegister.Type := PurchaseRegister.Type::Item
                            else
                                IF lPurchInvLine.Type = lPurchInvLine.Type::Resource then
                                    PurchaseRegister.Type := PurchaseRegister.Type::Resource;

            //16767 End

            //16767  PurchaseRegister.Type := Type;
            PurchaseRegister."No." := lPurchInvLine."No.";
            PurchaseRegister."Variant Code" := lPurchInvLine."Variant Code";
            PurchaseRegister."Item Description" := lPurchInvLine.Description;
            ...
            PurchaseRegister."Unit of Measure" := lPurchInvLine."Unit of Measure Code";
            ...
            PurchaseRegister.Quantity := lPurchInvLine.Quantity;
            IF lPurchInvHeader."Currency Code" <> '' THEN BEGIN
                PurchaseRegister."Line Discount Amount" := lPurchInvLine."Line Discount Amount" / lPurchInvHeader."Currency Factor";
                PurchaseRegister."Unit Price" := lPurchInvLine."Direct Unit Cost" / lPurchInvHeader."Currency Factor";
                PurchaseRegister.Amount := lPurchInvLine.Quantity * lPurchInvLine."Direct Unit Cost" / lPurchInvHeader."Currency Factor";
                PurchaseRegister."Line Amount" := lPurchInvLine."Line Amount" / lPurchInvHeader."Currency Factor";
            END ELSE BEGIN
                PurchaseRegister."Line Discount Amount" := lPurchInvLine."Line Discount Amount";
                PurchaseRegister."Unit Price" := lPurchInvLine."Direct Unit Cost";
                PurchaseRegister.Amount := lPurchInvLine.Quantity * lPurchInvLine."Direct Unit Cost";
                PurchaseRegister."Line Amount" := lPurchInvLine."Line Amount";
            END;
            ...
            PurchaseRegister."Custom Duty" := lPurchInvLine."Custom Duty Amount";//ACXRaman 031220
            PurchaseRegister."GST Assessable Value" := lPurchInvLine."GST Assessable Value";//ACXRaman 031220
            ...
            PurchaseRegister."GST Base Amount" := GstBaseAmt.GetGSTBaseAmtPostedLine(lPurchInvLine."Document No.", lPurchInvLine."Line No.");
            PurchaseRegister."Total GST Amount" := TotalGstAmt.GetTotalGSTAmtPostedLine(lPurchInvLine."Document No.", lPurchInvLine."Line No.");
            ...
            //16767 Start
            TdsEntry.Reset();
            TdsEntry.SetRange("Document No.", lPurchInvLine."Document No.");
            ...
            recDetGSTLedEntry.SETRANGE("Document No.", lPurchInvLine."Document No.");
            recDetGSTLedEntry.SETRANGE("Document Line No.", lPurchInvLine."Line No.");
            ...
            PurchaseRegister."Tax Group Code" := lPurchInvLine."Tax Group Code";
            PurchaseRegister."Tax Area" := lPurchInvLine."Tax Area Code";

            IF lPurchInvLine."Tax Area Code" <> '' THEN BEGIN
                /*  TaxAreaLine.RESET;
            ...
            PurchaseRegister."Line Discount %" := lPurchInvLine."Line Discount %";
            //16767 Start New Code
            if lPurchInvHeader."Applies-to Doc. Type" = lPurchInvHeader."Applies-to Doc. Type"::" " then
                PurchaseRegister."Applies-to Doc. Type" := PurchaseRegister."Applies-to Doc. Type"::" "
            else
                if lPurchInvHeader."Applies-to Doc. Type" = lPurchInvHeader."Applies-to Doc. Type"::"Credit Memo" then
                    PurchaseRegister."Applies-to Doc. Type" := PurchaseRegister."Applies-to Doc. Type"::"Credit Memo"
                else
                    if lPurchInvHeader."Applies-to Doc. Type" = lPurchInvHeader."Applies-to Doc. Type"::"Finance Charge Memo" then
                        PurchaseRegister."Applies-to Doc. Type" := PurchaseRegister."Applies-to Doc. Type"::"Finance Charge Memo"
                    else
                        if lPurchInvHeader."Applies-to Doc. Type" = lPurchInvHeader."Applies-to Doc. Type"::Invoice then
                            PurchaseRegister."Applies-to Doc. Type" := PurchaseRegister."Applies-to Doc. Type"::Invoice
                        else
                            if lPurchInvHeader."Applies-to Doc. Type" = lPurchInvHeader."Applies-to Doc. Type"::Payment then
                                PurchaseRegister."Applies-to Doc. Type" := PurchaseRegister."Applies-to Doc. Type"::Payment
                            else
                                if lPurchInvHeader."Applies-to Doc. Type" = lPurchInvHeader."Applies-to Doc. Type"::Refund then
                                    PurchaseRegister."Applies-to Doc. Type" := PurchaseRegister."Applies-to Doc. Type"::Refund
                                else
                                    if lPurchInvHeader."Applies-to Doc. Type" = lPurchInvHeader."Applies-to Doc. Type"::Reminder then
                                        PurchaseRegister."Applies-to Doc. Type" := PurchaseRegister."Applies-to Doc. Type"::Reminder;

            //16767 End Code
            //16767 PurchaseRegister."Applies-to Doc. Type" := lPurchInvHeader."Applies-to Doc. Type";
            PurchaseRegister."Applies-to Doc. No." := lPurchInvHeader."Applies-to Doc. No.";

            PurchaseRegister."Global Dimension 1 Code" := lPurchInvLine."Shortcut Dimension 1 Code";
            PurchaseRegister."Global Dimension 2 Code" := lPurchInvLine."Shortcut Dimension 2 Code";
            ...
            IF lPurchInvHeader."Currency Code" <> '' THEN
                ...
                PurchaseRegister."Net Amount" := AmtToVendor.GetAmttoVendorPostedDoc(lPurchInvLine."Document No.") / lPurchInvHeader."Currency Factor"
            ELSE
                //16767 PurchaseRegister."Net Amount" := "Amount To Vendor";
                PurchaseRegister."Net Amount" := AmtToVendor.GetAmttoVendorPostedDoc(lPurchInvLine."Document No.");
            ...
            IF (lPurchInvLine."Gen. Prod. Posting Group" <> '') AND (lPurchInvLine."Gen. Bus. Posting Group" <> '') THEN BEGIN
                IF GPS.GET(lPurchInvLine."Gen. Bus. Posting Group", lPurchInvLine."Gen. Prod. Posting Group") THEN BEGIN
        ...
        END;
        lPurchInvLine."Exported to Purch. Register" := TRUE;
    ...
    var
        ...
        SgstPer: Decimal;
        GetPer: Decimal;
        ApplytoDocType: Enum "Gen. Journal Document Type";
    begin



        //16767 WITH lPurchCrMemoLine DO BEGIN
        lPurchCrMemoHdr.GET(lPurchCrMemoLine."Document No.");
        //IF GenJnlTemplate.GET(lPurchCrMemoHdr."Gen. Journal Template Code") THEN;

        IF Loc.GET(lPurchCrMemoLine."Location Code") THEN;
        IF Vend.GET(lPurchCrMemoLine."Buy-from Vendor No.") THEN;
        IF VPG.GET(lPurchCrMemoHdr."Vendor Posting Group") THEN;
        //IF recCustPriceGroup.GET(lPurchCrMemoHdr."Customer Price Group") THEN;

        IF (lPurchCrMemoLine.Type = Type::"G/L Account") AND (lPurchCrMemoLine."No." = VPG."Invoice Rounding Account") THEN BEGIN
            PurchaseRegister1.RESET;
            PurchaseRegister1.SETRANGE("Document Type", PurchaseRegister1."Document Type"::"Purchase Cr. Memo");
            PurchaseRegister1.SETRANGE("Source Document No.", lPurchCrMemoLine."Document No.");
            IF PurchaseRegister1.FIND('+') THEN BEGIN //acxcp_110322 //code edit due to round off g/l error
                IF lPurchCrMemoHdr."Currency Code" <> '' THEN BEGIN
                    ...
                    PurchaseRegister1."Round Off Amount" := AmtToVendor.GetAmttoVendorPostedDoc(lPurchCrMemoLine."Document No.") / lPurchCrMemoHdr."Currency Factor";
                    PurchaseRegister1."Net Amount" += AmtToVendor.GetAmttoVendorPostedDoc(lPurchCrMemoLine."Document No.") / lPurchCrMemoHdr."Currency Factor";
                END ELSE BEGIN
                    //16767  PurchaseRegister1."Round Off Amount" := "Amount To Vendor";
                    //16767  PurchaseRegister1."Net Amount" += "Amount To Vendor";

                    PurchaseRegister1."Round Off Amount" := AmtToVendor.GetAmttoVendorPostedDoc(lPurchCrMemoLine."Document No.");
                    PurchaseRegister1."Net Amount" += AmtToVendor.GetAmttoVendorPostedDoc(lPurchCrMemoLine."Document No.");
                END;
        ...
        END ELSE BEGIN
            ...
            PurchaseRegister."Source Document No." := lPurchCrMemoLine."Document No.";
            PurchaseRegister."Source Line No." := lPurchCrMemoLine."Line No.";
            PurchaseRegister."Posting Date" := lPurchCrMemoHdr."Posting Date";
            PurchaseRegister."Source Line Description" := lPurchCrMemoLine.Description;
            ...
            PurchaseRegister."Source Type" := PurchaseRegister."Source Type"::Vendor;
            PurchaseRegister."Source No." := lPurchCrMemoLine."Buy-from Vendor No.";
            PurchaseRegister."Source Name" := Vend.Name;
            PurchaseRegister."Source City" := Vend.City;
            PurchaseRegister."Source State Code" := Vend."State Code";
            //ACX-RK 30092021 Begin
            ...
            PurchaseRegister."Customer Price Group Name" := recCustPriceGroup.Description;
            PurchaseRegister."Gen. Bus. Posting Group" := lPurchCrMemoLine."Gen. Bus. Posting Group";
            IF GBPG.GET(lPurchCrMemoLine."Gen. Bus. Posting Group") THEN;
            PurchaseRegister."GBPG Description" := GBPG.Description;
            IF GPPG.GET(lPurchCrMemoLine."Gen. Prod. Posting Group") THEN;
            PurchaseRegister."GPPG Description" := GPPG.Description;
            PurchaseRegister."Country/Region Code" := Vend."Country/Region Code";
            IF Vend."Country/Region Code" <> '' THEN BEGIN
                Country.GET(Vend."Country/Region Code");
            ...
            */
            ...
            PurchaseRegister."Outward Location Code" := lPurchCrMemoLine."Location Code";
            PurchaseRegister."Outward State Code" := Loc."State Code";
            IF recState.GET(Loc."State Code") THEN BEGIN
                PurchaseRegister."Outward State Name" := recState.Description;
            END;
            ...
            IF (lPurchCrMemoLine.Type = Type::Item) AND (lPurchCrMemoLine."No." <> '') THEN BEGIN
                Item.GET(lPurchCrMemoLine."No.");
                //16767  PurchaseRegister."Excise Prod. Posting Group" := Item."Excise Prod. Posting Group";
                PurchaseRegister."Inventory Posting Group" := Item."Inventory Posting Group";
                PurchaseRegister."Gen. Prod. Posting Group" := Item."Gen. Prod. Posting Group";
            ...
            //16767 New Code add
            IF lPurchCrMemoLine.Type = lPurchCrMemoLine.Type::" " then
                PurchaseRegister.Type := PurchaseRegister.Type::" "
            else
                IF lPurchCrMemoLine.Type = lPurchCrMemoLine.Type::"Charge (Item)" then
                    PurchaseRegister.Type := PurchaseRegister.Type::"Charge (Item)"
                else
                    IF lPurchCrMemoLine.Type = lPurchCrMemoLine.Type::"Fixed Asset" then
                        PurchaseRegister.Type := PurchaseRegister.Type::"Fixed Asset"
                    else
                        IF lPurchCrMemoLine.Type = lPurchCrMemoLine.Type::"G/L Account" then
                            PurchaseRegister.Type := PurchaseRegister.Type::"G/L Account"
                        else
                            IF lPurchCrMemoLine.Type = lPurchCrMemoLine.Type::Item then
                                PurchaseRegister.Type := PurchaseRegister.Type::Item
                            else
                                IF lPurchCrMemoLine.Type = lPurchCrMemoLine.Type::Resource then
                                    PurchaseRegister.Type := PurchaseRegister.Type::Resource;

            //16767 End

            //16767  PurchaseRegister.Type := Type;
            PurchaseRegister."No." := lPurchCrMemoLine."No.";
            PurchaseRegister."Variant Code" := lPurchCrMemoLine."Variant Code";
            PurchaseRegister."Item Description" := lPurchCrMemoLine.Description;
            PurchaseRegister."Gross Weight" := Item."Gross Weight";
            PurchaseRegister."Net Weight" := Item."Net Weight";
            PurchaseRegister."Units per Parcel" := Item."Units per Parcel";
            PurchaseRegister."Unit of Measure" := lPurchCrMemoLine."Unit of Measure Code";
            ...
            PurchaseRegister.Quantity := -lPurchCrMemoLine.Quantity;

            IF lPurchCrMemoHdr."Currency Code" <> '' THEN BEGIN
                PurchaseRegister."Line Discount Amount" := -lPurchCrMemoLine."Line Discount Amount" / lPurchCrMemoHdr."Currency Factor";
                PurchaseRegister."Unit Price" := -lPurchCrMemoLine."Direct Unit Cost" / lPurchCrMemoHdr."Currency Factor";
                PurchaseRegister.Amount := -lPurchCrMemoLine.Quantity * lPurchCrMemoLine."Direct Unit Cost" / lPurchCrMemoHdr."Currency Factor";
                PurchaseRegister."Line Amount" := -lPurchCrMemoLine."Line Amount" / lPurchCrMemoHdr."Currency Factor";
            END ELSE BEGIN
                PurchaseRegister."Line Discount Amount" := -lPurchCrMemoLine."Line Discount Amount";
                PurchaseRegister."Unit Price" := -lPurchCrMemoLine."Direct Unit Cost";
                PurchaseRegister.Amount := -lPurchCrMemoLine.Quantity * lPurchCrMemoLine."Direct Unit Cost";
                PurchaseRegister."Line Amount" := -lPurchCrMemoLine."Line Amount";
            END;
            ...
            PurchaseRegister."Tax Group Code" := lPurchCrMemoLine."Tax Group Code";
            PurchaseRegister."Tax Area" := lPurchCrMemoLine."Tax Area Code";

            IF lPurchCrMemoLine."Tax Area Code" <> '' THEN BEGIN
                /* TaxAreaLine.RESET;
                TaxAreaLine.SETRANGE("Tax Area", "Tax Area Code");
                TaxAreaLine.FIND('-');
                PurchaseRegister."Tax Jurisdiction Code" := TaxAreaLine."Tax Jurisdiction Code";
            ...
            PurchaseRegister."Line Discount %" := lPurchCrMemoLine."Line Discount %";
            if lPurchCrMemoHdr."Applies-to Doc. Type" = lPurchCrMemoHdr."Applies-to Doc. Type"::" " then
                PurchaseRegister."Applies-to Doc. Type" := PurchaseRegister."Applies-to Doc. Type"::" "
            else
                if lPurchCrMemoHdr."Applies-to Doc. Type" = lPurchCrMemoHdr."Applies-to Doc. Type"::"Credit Memo" then
                    PurchaseRegister."Applies-to Doc. Type" := PurchaseRegister."Applies-to Doc. Type"::"Credit Memo"
                else
                    if lPurchCrMemoHdr."Applies-to Doc. Type" = lPurchCrMemoHdr."Applies-to Doc. Type"::"Finance Charge Memo" then
                        PurchaseRegister."Applies-to Doc. Type" := PurchaseRegister."Applies-to Doc. Type"::"Finance Charge Memo"
                    else
                        if lPurchCrMemoHdr."Applies-to Doc. Type" = lPurchCrMemoHdr."Applies-to Doc. Type"::Invoice then
                            PurchaseRegister."Applies-to Doc. Type" := PurchaseRegister."Applies-to Doc. Type"::Invoice
                        else
                            if lPurchCrMemoHdr."Applies-to Doc. Type" = lPurchCrMemoHdr."Applies-to Doc. Type"::Payment then
                                PurchaseRegister."Applies-to Doc. Type" := PurchaseRegister."Applies-to Doc. Type"::Payment
                            else
                                if lPurchCrMemoHdr."Applies-to Doc. Type" = lPurchCrMemoHdr."Applies-to Doc. Type"::Refund then
                                    PurchaseRegister."Applies-to Doc. Type" := PurchaseRegister."Applies-to Doc. Type"::Refund
                                else
                                    if lPurchCrMemoHdr."Applies-to Doc. Type" = lPurchCrMemoHdr."Applies-to Doc. Type"::Reminder then
                                        PurchaseRegister."Applies-to Doc. Type" := PurchaseRegister."Applies-to Doc. Type"::Reminder;

            //16767 End Code
            //16767  PurchaseRegister."Applies-to Doc. Type" := lPurchCrMemoHdr."Applies-to Doc. Type";
            PurchaseRegister."Applies-to Doc. No." := lPurchCrMemoHdr."Applies-to Doc. No.";

            PurchaseRegister."Global Dimension 1 Code" := lPurchCrMemoLine."Shortcut Dimension 1 Code";
            PurchaseRegister."Global Dimension 2 Code" := lPurchCrMemoLine."Shortcut Dimension 2 Code";
            //16767 PurchaseRegister."T.I.N. No." := Vend."T.I.N. No.";
            ...
            IF lPurchCrMemoHdr."Currency Code" <> '' THEN
                ...
                PurchaseRegister."Net Amount" := AmtToVendor.GetAmttoVendorPostedDoc(lPurchCrMemoLine."Document No.") / lPurchCrMemoHdr."Currency Factor"
            ELSE
                //16767  PurchaseRegister."Net Amount" := "Amount To Vendor";
                PurchaseRegister."Net Amount" := AmtToVendor.GetAmttoVendorPostedDoc(lPurchCrMemoLine."Document No.");
            ...
            Clear(GetPer);
            recDetGSTLedEntry.RESET();
            recDetGSTLedEntry.SETRANGE("Document No.", lPurchCrMemoLine."Document No.");
            recDetGSTLedEntry.SETRANGE("Document Line No.", lPurchCrMemoLine."Line No.");
            IF recDetGSTLedEntry.FindSet() THEN begin
                repeat
                    IF recDetGSTLedEntry."GST Component Code" = 'IGST' THEN
                        IgstPer := recDetGSTLedEntry."GST %";
            ...
            IF (lPurchCrMemoLine."Gen. Prod. Posting Group" <> '') AND (lPurchCrMemoLine."Gen. Bus. Posting Group" <> '') THEN BEGIN
                IF GPS.GET(lPurchCrMemoLine."Gen. Bus. Posting Group", lPurchCrMemoLine."Gen. Prod. Posting Group") THEN BEGIN
                    PurchaseRegister."Revenue Account Code" := GPS."Sales Account";
                    IF GLAcc.GET(GPS."Sales Account") THEN
                        PurchaseRegister."Revenue Account Description" := GLAcc.Name;
                END;
            ...
            PurchaseRegister.INSERT;
        END;
        lPurchCrMemoLine."Exported to Purch. Register" := TRUE;
        lPurchCrMemoLine.MODIFY;
    ...
    begin
        ...
        IF lTransRcptLine.FIND('-') THEN BEGIN
            REPEAT
                //16767   WITH lTransRcptLine DO BEGIN
                Loc.GET(lTransRcptLine."Transfer-from Code");
                Loc2.GET(lTransRcptLine."Transfer-to Code");
                PurchaseRegister.INIT;
                ...
                PurchaseRegister."Source Document No." := lTransRcptLine."Document No.";
                PurchaseRegister."Source Line No." := lTransRcptLine."Line No.";
                PurchaseRegister."Posting Date" := lTransRcptHdr."Posting Date";
                PurchaseRegister."Source Line Description" := lTransRcptLine.Description;
                ...
                //    PurchaseRegister."CPG Desc" := VPG.Description ;
                PurchaseRegister."Customer Price Group Name" := recCustPriceGroup.Description;
                GPPG.GET(lTransRcptLine."Gen. Prod. Posting Group");
                PurchaseRegister."GPPG Description" := GPPG.Description;
                ...
                PurchaseRegister."No." := lTransRcptLine."Item No.";
                PurchaseRegister."Variant Code" := lTransRcptLine."Variant Code";
                PurchaseRegister."Item Description" := lTransRcptLine.Description;
                PurchaseRegister."Gross Weight" := Item."Gross Weight";
                PurchaseRegister."Global Dimension 1 Code" := lTransRcptLine."Shortcut Dimension 1 Code";
                PurchaseRegister."Global Dimension 2 Code" := lTransRcptLine."Shortcut Dimension 2 Code";

                PurchaseRegister."Net Weight" := Item."Net Weight";
                PurchaseRegister."Units per Parcel" := Item."Units per Parcel";
                PurchaseRegister."Unit of Measure" := lTransRcptLine."Unit of Measure Code";
                ...
                IF (lTransRcptLine."Item No." <> '') THEN BEGIN
                    Item.GET(lTransRcptLine."Item No.");
                    //16767  PurchaseRegister."Excise Prod. Posting Group" := Item."Excise Prod. Posting Group";
                    PurchaseRegister."Inventory Posting Group" := Item."Inventory Posting Group";
                    PurchaseRegister."Gen. Prod. Posting Group" := Item."Gen. Prod. Posting Group";
                ...
                END;
                PurchaseRegister.Quantity := lTransRcptLine.Quantity;
                PurchaseRegister."Unit Price" := lTransRcptLine."Unit Price";
                PurchaseRegister.Amount := (lTransRcptLine.Quantity * lTransRcptLine."Unit Price");
                ...
                PurchaseRegister."Net Amount" := lTransRcptLine.Amount + /* "Excise Amount" */ +decTD + decCD + decSD + decOffSeason + decFreight;//16767

                IF (lTransRcptHdr."Transfer-from Code" <> '') AND (lTransRcptHdr."Transfer-to Code" <> '') AND
                                     (lTransRcptLine."Gen. Prod. Posting Group" <> '') THEN BEGIN
                ...
                PurchaseRegister.INSERT;
                lTransRcptLine."Exported to Purch. Register" := TRUE;
                lTransRcptLine.MODIFY;
            //16767 END;
