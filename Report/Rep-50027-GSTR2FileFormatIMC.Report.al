report 50027 "GSTR-2 File Format-IMC"
{
    ...
    dataset
    {
        dataitem(Integer; Integer)
        {
            ...
            begin
                ...
                MakeExcelBodyB2BUR;//no change
                CreateExcel(B2BURTxt);

                MakeExcelBodyIMPS;//no chnage
                CreateExcel(IMPSTxt);

                MakeExcelBodyIMPG;//no change
                CreateExcel(IMPGTxt);

                MakeExcelBodyAT; //no change
                CreateExcel(ATTxt);

                MakeExcelBodyCDNR; // CHANGE DONE
                CreateExcel(CDNRTxt);

                MakeExcelBodyCDNUR;//NO CHANGE
                CreateExcel(CDNURTxt);

                //  MakeExcelBodyEXEMP;//16767 Name Replace
                MakeExcelHeaderEXEMP;
                CreateExcel(EXEMPTxt);
                //name replace MakeExcelBodyHSN
                MakeExcelBodyHSNSUM;
                CreateExcel(HSNSUMTxt);
    ...
    /*  local procedure MakeExcelBodyB2B()
     ...
     begin
         ...
         GSTR2B2BQuery.SetRange(Entry_Type, GSTR2B2BQuery.Entry_Type::"Initial Entry"); //16767 New Change
    ...
    begin
        MakeExcelHeaderB2B();
        GSTR2B2BQuery.SetRange(Location__Reg__No_, LocationGSTIN);
        GSTR2B2BQuery.SetFilter(Entry_Type, '%1', GSTR2B2BQuery.Entry_Type::"Initial Entry"); //16767 New Change
        GSTR2B2BQuery.SetRange(Posting_Date, StartDate, EndDate);
        GSTR2B2BQuery.SetFilter(GST_Vendor_Type, '%1|%2', "GST Vendor Type"::Registered, "GST Vendor Type"::SEZ);
    ...
    begin
        //16767 New Code Add
        if Vendor.Get(GSTR2B2BQuery.Source_No_) then begin
            recCompInfo.Get();
            //16767 Code Add
            TempExcelBuffer.NewRow();
            TempExcelBuffer.AddColumn(GSTR2B2BQuery.Buyer_Seller_Reg__No_, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
    ...
    begin
        ...
        TempExcelBuffer.AddColumn(GSTINUINTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn(SupplierNameTxt, FALSE, '', TRUE, FALSE, FALSE, '', TempExcelBuffer."Cell Type"::Text); //16767 NEW CODE ADD
    ...
    begin
        Vendor.Get(GSTR2CDNRQuery.Source_No_);//16767 NEW CODE ADD
        TempExcelBuffer.NewRow();
    ...
    var
        ...
        GSTBaseAmt: Decimal;
    begin
        MakeExcelHeaderEXEMP();//16767 Change
