report 50002 "Update Purchase Register"
{
    ...
    dataset
    {
        dataitem(DataItem1000000000; Table123)
        {
            DataItemTableView = SORTING (Document No., Line No.)
                                WHERE (Exported to Purch. Register=FILTER(No),
        ...
        dataitem(DataItem1000000001;Table125)
        {
            DataItemTableView = SORTING(Document No.,Line No.)
                                WHERE(Exported to Purch. Register=FILTER(No),
                                      Buy-from Vendor No.=FILTER(<>''));
        ...
        dataitem(DataItem1000000002;Table5746)
        {
    ...
    var
        ...
        recPostedStrOrderDetail: Record "13798";
        Loc2: Record "14";
        recExcisePostingSetup: Record "13711";
        recServiceTaxSetup: Record "16472";
        decCustom: Decimal;
        ...
        recPostedStrOrdrDtl: Record "13760";
    ...
    begin
        ...
        WITH lPurchInvLine DO BEGIN
          ...
          //IF GenJnlTemplate.GET(lPurchInvHeader."Gen. Journal Template Code") THEN;
           ...
           // IF (Type = Type::"G/L Account") AND ("No." = VPG."Invoice Rounding Account") AND ("Line No."<>10000) THEN BEGIN
              ...
              PurchaseRegister1."Round Off Amount" := "Amount To Vendor"/lPurchInvHeader."Currency Factor" ;
              PurchaseRegister1."Net Amount" += "Amount To Vendor"/lPurchInvHeader."Currency Factor" ;
              END ELSE BEGIN
              PurchaseRegister1."Round Off Amount" := "Amount To Vendor" ;
              PurchaseRegister1."Net Amount" += "Amount To Vendor" ;
              END;
           ...
           END ELSE BEGIN
              ...
              IF lPurchInvHeader."Order No."<>'' THEN BEGIN
                ...
                ELSE BEGIN
                  recPurRecipt.RESET;
                  recPurRecipt.SETRANGE("Document No.",lPurchInvLine."Receipt Document No.");
                  recPurRecipt.SETRANGE("No.", lPurchInvLine."No.");
                  recPurRecipt.SETRANGE("Line No.",lPurchInvLine."Receipt Document Line No.");
        ...
        //      PurchaseRegister."Customer Price Group"   := lPurchInvHeader."Customer Price Group";
             ...
             // PurchaseRegister."Location Type" := Loc."Location Type";
              ...
              IF intMonth < 4 THEN BEGIN
                 ...
                 cdQuarter := 'Q4';
                 END
              ELSE IF intMonth < 7 THEN BEGIN
                 cdFinYear := FORMAT(intYear) + '-' + FORMAT(intYear-2000+1);
                 cdQuarter := 'Q1';
                 END
              ELSE IF intMonth < 10 THEN BEGIN
                 cdFinYear := FORMAT(intYear) + '-' + FORMAT(intYear-2000+1);
                 cdQuarter := 'Q2';
                 END
              ELSE BEGIN
              ...
              IF (Type = Type::Item) AND ("No." <> '') THEN BEGIN
                 Item.GET("No.") ;
                 PurchaseRegister."Excise Prod. Posting Group" := Item."Excise Prod. Posting Group" ;
              ...
              PurchaseRegister."GST Base Amount":= "GST Base Amount"; //ACXKD 071220
              PurchaseRegister."Total GST Amount":= "Total GST Amount";//ACXKD 071220
             ...
             END;
              ...
              PurchaseRegister."TDS Base Amount" := "Tax Base Amount";
        ...
        //            PurchaseRegister."TCS Details Required" := recPostedStrOrderDetail."Calculation Value";
              ...
              PurchaseRegister."Tax Amount" := "Tax Amount" ;
              ...
              PurchaseRegister."Form Code" := lPurchInvHeader."Form Code" ;
              PurchaseRegister."Form No." := lPurchInvHeader."Form No.";
              ...
              PurchaseRegister."T.I.N. No." := Vend."T.I.N. No.";
              PurchaseRegister."No. Series" := lPurchInvHeader."No. Series" ;
              IF lPurchInvHeader."Currency Code"<>'' THEN
              PurchaseRegister."Net Amount" := "Amount To Vendor"/lPurchInvHeader."Currency Factor"
              ELSE
              PurchaseRegister."Net Amount" := "Amount To Vendor";
    ...
    begin
         ...
         //IF GenJnlTemplate.GET(lPurchCrMemoHdr."Gen. Journal Template Code") THEN;
           ...
           IF (Type = Type::"G/L Account") AND ("No." = VPG."Invoice Rounding Account") THEN BEGIN
              ...
              IF PurchaseRegister1.FIND('+') THEN BEGIN //acxcp_110322 //code edit due to round off g/l error
                ...
                PurchaseRegister1."Round Off Amount" := "Amount To Vendor"/lPurchCrMemoHdr."Currency Factor" ;
                PurchaseRegister1."Net Amount" += "Amount To Vendor"/lPurchCrMemoHdr."Currency Factor" ;
                END ELSE BEGIN
                PurchaseRegister1."Round Off Amount" := "Amount To Vendor" ;
                PurchaseRegister1."Net Amount" += "Amount To Vendor" ;
                END;
        ...
        //AR//
              ...
              IF PurchaseRegister."Tax Type" = PurchaseRegister."Tax Type"::" " THEN
                 PurchaseRegister."Tax Type" := PurchaseRegister."Tax Type"::VAT;
              PurchaseRegister."Form Code" := lPurchCrMemoHdr."Form Code" ;
              PurchaseRegister."Form No." := lPurchCrMemoHdr."Form No.";
              PurchaseRegister."Line Discount %" := "Line Discount %";
              ...
              PurchaseRegister."Net Amount" := "Amount To Vendor"/lPurchCrMemoHdr."Currency Factor"
              ELSE
              PurchaseRegister."Net Amount" := "Amount To Vendor";
          ...
          //    PurchaseRegister."CPG Desc" := VPG.Description ;
            ...
            //  PurchaseRegister."Location Type" := Loc."Location Type";
             ...
             END;
              ...
              PurchaseRegister."Net Amount" := Amount + "Excise Amount" +decTD+decCD+decSD+decOffSeason+decFreight  ;
