report 50029 "GSTR 6"
{
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    field(FileFormat; TypeofSupply)
                    {
                        ...
                        ApplicationArea = all;
                        Visible = false; //16767 New Property Add
                    }
                }
            }
        }
    ...
    var
        ...
        TotalValue: Decimal;
        //1676 NEw Add
        Status_of_GSTRTxt: Label 'ENU=Status of GSTR1/5';
        invoice_StatusTxt: Label 'ENU=Invoice Status';
        ActionTxt: Label 'ENU=Action';
        Sheet_Validation_ErrorsTxt: Label 'ENU=Sheet Validation Errors';
        GST_Portal_Validation_ErrorsTxt: Label 'ENU="GST Portal Validation Errors"';
        Note_TypeTxt: Label 'ENU=Note Type';
        DebitCreditNoteNoTxt: Label 'ENU=Debit/Credit Note No.';
        DebitCreditNoteDateTxt: Label 'ENU=Debit/Credit Note Date (DD-MM-YYYY)';
        DebitCreditNoteValueTxt: Label 'ENU=Debit/Credit Note Value ( Rs )';
        Original_invoice_numberTxt: Label 'ENU=Original invoice number';
        Original_invoice_dateTxt: Label 'ENU="Original invoice date  (dd-mm-yyyy) "';
        Supply_TypeTxt: Label 'ENU=Supply Type';
        Reason_for_issuing_noteTxt: Label 'ENU=Reason for issuing note';
        Filing_status_of_GSTR: Label 'ENU=Filing status of GSTR1/5';
        Debit_Credit_note_status: Label 'ENU=Debit/Credit note status';
        SupplierNameTxt1: Label 'ENU=Supplier Name';
        recPosPurHead: Record 122;
        DocNoTxt: Label 'ENU=Document Number';
        decInvAmt: Decimal;
        GSTAmountValue: Decimal;
        IGSTAmountTxt: Label 'ENU=IGST Amount;ENN=IGST Amount';
        CGSTAmountTxt: Label 'ENU=CGST Amount;ENN=CGST Amount';
        SGSTAmountTxt: Label 'ENU=SGST Amount;ENN=SGST Amount';
    ...
    begin
        ...
        //16767 NEw Add
        TempExcelBuffer.AddColumn(SupplierNameTxt1, FALSE, '', TRUE, FALSE, FALSE, '', TempExcelBuffer."Cell Type"::Text);
        ...
        //16767  TempExcelBuffer.AddColumn(InvoiceDateTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        ...
        //16767 TempExcelBuffer.AddColumn(InvoiceTypeTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        //16767 NEw
        TempExcelBuffer.AddColumn(Supply_TypeTxt, FALSE, '', TRUE, FALSE, FALSE, '', TempExcelBuffer."Cell Type"::Text);
        //16767 End
        ...
        /*  TempExcelBuffer.AddColumn(IntegratedTaxPaidTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
         ...
         TempExcelBuffer.AddColumn(AvailedITCCessTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text); */ //16767
    end;
                 ...
                 if (GSTR2B2BQuery.GST_Vendor_Type = GSTR2B2BQuery.GST_Vendor_Type::SEZ) and (GSTR2B2BQuery.GST_Group_Type = GSTR2B2BQuery.GST_Group_Type::Goods) and (GSTR2B2BQuery.Without_Bill_Of_Entry = true) then begin
                     ...
                     if (GSTR2B2BQuery.GST_Vendor_Type = GSTR2B2BQuery.GST_Vendor_Type::SEZ) and (GSTR2B2BQuery.GST_Group_Type = GSTR2B2BQuery.GST_Group_Type::Service) then
                         if (GSTR2B2BQuery.Reverse_Charge = true) and (GSTR2B2BQuery.Entry_Type = GSTR2B2BQuery.Entry_Type::Application) then
                             ...
                             if (GSTR2B2BQuery.Reverse_Charge = false) and (GSTR2B2BQuery.Entry_Type = GSTR2B2BQuery.Entry_Type::"Initial Entry") then
                                 CreateExcelBufferB2B(GSTR2B2BQuery);
     end; */ //16767 new function add

    //16767 Start
    local procedure MakeExcelBodyB2B()
    var
        GSTR2B2BQuery: Query GSTR2B2BQuery1;
    begin
        MakeExcelHeaderB2B();

        GSTR2B2BQuery.SetRange(Location__Reg__No_, LocationGSTIN);
        GSTR2B2BQuery.SetRange(GSTR2B2BQuery.GSTled_Input_Service_Dist_, true); //16767 new 
        GSTR2B2BQuery.SetFilter(GSTR2B2BQuery.GST_Credit, '<>%1', "GST Credit"::"Non-Availment");//16767 new add
        GSTR2B2BQuery.SetRange(Posting_Date, StartDate, EndDate);
        GSTR2B2BQuery.SetFilter(GST_Vendor_Type, '%1|%2', "GST Vendor Type"::Registered, "GST Vendor Type"::SEZ);
        GSTR2B2BQuery.SetFilter(GST_Group_Type, '%1|%2', "GST Group Type"::Goods, "GST Group Type"::Service);
        GSTR2B2BQuery.SetFilter(GST__, '<> %1', 0);
    ...
    //New code add 16767
    Local procedure CreateExcelBufferB2B(GSTR2B2BQuery: Query GSTR2B2BQuery1)
    var
        State: Record State;
    ...
    begin
        vendor.Get(GSTR2B2BQuery.Source_No_);//16767 new add
        TempExcelBuffer.NewRow();
        ...
        recPosPurHead.RESET();
        recPosPurHead.SETRANGE("No.", GSTR2B2BQuery.Document_No_);
        IF recPosPurHead.FINDFIRST THEN;
        TempExcelBuffer.AddColumn(recPosPurHead."No.", FALSE, '', FALSE, FALSE, FALSE, '', TempExcelBuffer."Cell Type"::Text);//acxcp211221

        TempExcelBuffer.AddColumn(recPosPurHead."Document Date", FALSE, '', FALSE, FALSE, FALSE, '', TempExcelBuffer."Cell Type"::Text);

        TempExcelBuffer.AddColumn(GSTR2B2BQuery.External_Document_No_, FALSE, '', FALSE, FALSE, FALSE, '', TempExcelBuffer."Cell Type"::Text);
        ...
        /*   if GSTR2B2BQuery.Reverse_Charge then
              ...
              TempExcelBuffer.AddColumn(NTxt, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text); */ //16767 Comment in live Server
                                                                                                                         //16767 New Code Add
                                                                                                                         //GST Jurisdiction
        TempExcelBuffer.AddColumn(
         GSTR2B2BQuery.GST_Jurisdiction_Type, FALSE, '', FALSE, FALSE, FALSE, '', TempExcelBuffer."Cell Type"::Text);
        //16767 end
    ...
    local procedure GetInvoiceType(GSTR2B2BQuery: Query GSTR2B2BQuery1): Text[50]
    begin
        case GSTR2B2BQuery.GST_Vendor_Type of
            GSTR2B2BQuery.GST_Vendor_Type::Registered:
                exit(RegularTxt);
    ...
    begin
        ...
        //16767 End
        TempExcelBuffer.AddColumn(RefundVoucherNoTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        //16767   TempExcelBuffer.AddColumn(RefundVoucherDateTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        //16767 NEw add
        ...
        TempExcelBuffer.AddColumn(PlaceOfSupplyTxt, FALSE, '', TRUE, FALSE, FALSE, '', TempExcelBuffer."Cell Type"::Text);
        //16767 End
        // 16767 comment in live TempExcelBuffer.AddColumn(InvVoucherNoTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        //16767 comment in live TempExcelBuffer.AddColumn(InvVoucherDateTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        //16767 comment in live TempExcelBuffer.AddColumn(PreGSTTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        // 16767 comment in live TempExcelBuffer.AddColumn(DocumentTypeTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn(ReasonForIssuingNoteTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn(SupplyTypeTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        //16767 comment in live TempExcelBuffer.AddColumn(RefundVoucherValueTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn(RateTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn(TaxableValueTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn(IntegratedTaxPaidTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn(CentralTaxPaidTxt, false, '', true, false, false, '', TempExcelBuffer."Cell Type"::Text);
 ...
 */
    ...
    local procedure MakeExcelBodyCDNR()
    var
        GSTR2CDNRQuery: Query GSTR2CDNRQuery1;
    begin

        MakeExcelHeaderCDNR();
        GSTR2CDNRQuery.SetRange(GSTR2CDNRQuery.GSTled_Input_Service_Dist_, true);//16767
    ...
    /*  local procedure CreateExcelBufferCDNR(GSTR2CDNRQuery: Query GSTR2CDNRQuery)
     ...
     begin
         ...
         if (CessAmountCrAvailed <> 0) then
             ...
             //16767 START
    local procedure CreateExcelBufferCDNR(GSTR2CDNRQuery: Query GSTR2CDNRQuery1)
    var
        ReferenceInvoiceNo: Record "Reference Invoice No.";
        GSTR2GSTPer: Query GSTR2GSTPer;
        GSTR2IGSTAmt: Query GSTR2IGST;
    ...
    begin
        IF State.GET(GSTR2CDNRQuery.Buyer_Seller_State_Code) THEN;
        if Vendor.Get(GSTR2CDNRQuery.Source_No_) then
            TempExcelBuffer.NewRow();
        TempExcelBuffer.AddColumn(GSTR2CDNRQuery.Buyer_Seller_Reg__No_, false, '', false, false, false, '', TempExcelBuffer."Cell Type"::Text);
        //16767 Start
        TempExcelBuffer.AddColumn(Vendor.Name, FALSE, '', FALSE, FALSE, FALSE, '', TempExcelBuffer."Cell Type"::Text);
        if GSTR2CDNRQuery.Original_Doc__Type = GSTR2CDNRQuery.Original_Doc__Type::"Credit Memo" then
        ...
        TempExcelBuffer.AddColumn(GSTR2CDNRQuery.External_Document_No_, FALSE, '', FALSE, FALSE, FALSE, '', TempExcelBuffer."Cell Type"::Text);
        ...
        TempExcelBuffer.AddColumn(ABS(GSTR2CDNRQuery.GST_Base_Amount), FALSE, '', FALSE, FALSE, FALSE, '', TempExcelBuffer."Cell Type"::Text);

        //Original Invoice No.
        TempExcelBuffer.AddColumn(GSTR2CDNRQuery.Original_Invoice_No_, FALSE, '', FALSE, FALSE, FALSE, '', TempExcelBuffer."Cell Type"::Text);

        //Original Invoice Date
        TempExcelBuffer.AddColumn(GSTR2CDNRQuery.Original_Invoice_Date, FALSE, '', FALSE, FALSE, FALSE, '', TempExcelBuffer."Cell Type"::Text);
    ...
    LOCAL PROCEDURE GetInvAmt(DetailedGSTLedgerEntry: Record "Detailed GST Ledger Entry"): Decimal;
    VAR
        recPurInvH: Record 123;
        GSTAmountValue3: Decimal;
    BEGIN
        recPurInvH.RESET;
